...
void* GetMemory(void* arg) {
  ...
  MemoryPool *mp = static_cast<MemoryPool*>(k->mp);
  char **pp = static_cast<char**>(k->p);
  cout << "pp:" << reinterpret_cast<void*>(pp) << endl;
  ...
  while (i < 100) {
    ...
    size = rand_r(&seed) % 1000 + 1;
    *pp = static_cast<char*>(mp->KCalloc(size));
    pp = pp + 1;
    ++i;
  }
  return NULL;
...
int main() {
  ...
  // new a instance in specified place
  Test *t = new(mp->KMalloc(sizeof(Test))) Test();
  cout <<"address of t is: " <<reinterpret_cast<void*>(t) <<endl;
  t->Print();
  cout <<mp->KMalloc(121) <<endl;

  {  // testing multi-thread
    char *p[1000];
    memset(p, 0, sizeof(p));
    cout << "p[0]:" << reinterpret_cast<void*>(p[0])
        << " p:" << reinterpret_cast<void*>(p) << endl;
    kk k1, k2, k3, k4, k5;
    ...
    k1.index_pp = 0;
    k1.p = p;
    cout << "k1.p:" << reinterpret_cast<void*>(k1.p) << endl;
    pthread_create(&p1, NULL, GetMemory, &k1);

    k2.mp = mp;
    k2.index_pp = 100;
    k2.p = p + 100;
    cout << "k2.p:" << reinterpret_cast<void*>(k2.p) << endl;
    pthread_create(&p2, NULL, GetMemory, &k2);

    k3.mp = mp;
    k3.index_pp = 200;
    k3.p = p + 200;
    cout << "k3.p:" << reinterpret_cast<void*>(k3.p) << endl;
    pthread_create(&p3, NULL, GetMemory, &k3);

    k4.mp = mp;
    k4.index_pp = 300;
    k4.p = p + 300;
    cout << "k4.p:" << reinterpret_cast<void*>(k4.p) << endl;
    pthread_create(&p4, NULL, GetMemory, &k4);

    k5.mp = mp;
    k5.index_pp = 400;
    k5.p = p + 400;
    cout << "k5.p:" << reinterpret_cast<void*>(k5.p) << endl;
    ...
    sort(p, p + 499);
    for (int i = 0; i < 500; ++i) {
      cout << reinterpret_cast<void*>(p[i]) << endl;
      if (i > 0 && p[i] == p[i - 1]) {
        cout << endl << "same!!!!!!!!!!!!!" << endl;
        break;
      }
