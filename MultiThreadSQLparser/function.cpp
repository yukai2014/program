...
{
	struct Table * a= (struct Table *)malloc(sizeof(struct Table));
	if(!a) {
		yyerror("out of space!");
		exit(0);
	}
...
void output(Node * oldnode, int floor)
{
	...
	switch(type)
	{
		case t_name_name:
		{
			...
			outputSpace(floor);
			cout<<"Column: "<< node->parameter1<< "  "<< node->parameter2<<endl;
			break;
		}
		case t_name:
		case t_stringval:
		...
		case t_expr_list:
		{
			...
			cout<<"Expr_list: ";
			/*
			if(node->data) output(node->data);
			if(node->next) output(node->next);
			*/
